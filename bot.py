...
BOT_SECRET = "aricToFame_2024_SecureKey_x7K9mP2qL5vN8wR3"
SPAM_LIMIT = 5
SPAM_WINDOW = 10
...
def verify_request(update: Update) -> bool:
    ...
    if user_id in blacklisted_users:
        if now < blacklisted_users[user_id]:
            return False
        else:
    ...
    if len(user_message_times[user_id]) >= SPAM_LIMIT:
        blacklisted_users[user_id] = now + BLACKLIST_DURATION.total_seconds()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = get_user_kb(user_id)
    if is_owner(user_id):
        await update.message.reply_text("👑 Панель владельца активирована!", reply_markup=kb)
    elif is_admin(user_id):
        await update.message.reply_text("🛡️ Админ-панель активирована!", reply_markup=kb)
    elif is_moder(user_id):
        await update.message.reply_text("🔧 Панель модератора активирована!", reply_markup=kb)
    else:
        await update.message.reply_text("✨ Добро пожаловать в Aricto Fame!", reply_markup=kb)
...
async def app_avatar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✏️ Введите никнейм:", reply_markup=get_cancel_keyboard())
    return APP_NICKNAME
...
async def app_nickname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔗 Введите ссылку на проект:", reply_markup=get_cancel_keyboard())
    return APP_PROJECT
...
async def app_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💬 Введите ссылку на чат (или '-'):", reply_markup=get_cancel_keyboard())
    return APP_CHAT
...
async def app_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📅 С какого года в КМ?", reply_markup=get_cancel_keyboard())
    return APP_KM_YEAR
...
async def app_km_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎯 Участвовали в ВК/ДС КМ? (да/нет/подробнее)", reply_markup=get_cancel_keyboard())
...
async def app_participated(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💭 Почему хотите к нам? (или '-'):", reply_markup=get_cancel_keyboard())
    return APP_REASON
...
async def app_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📈 Как поднимали фейм?", reply_markup=get_cancel_keyboard())
...
async def app_fame_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👥 С кем знакомы?", reply_markup=get_cancel_keyboard())
...
async def complaint_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📝 Опишите причину:", reply_markup=get_cancel_keyboard())
...
async def complaint_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📎 Прикрепите доказательства (текст/фото/видео):", reply_markup=get_cancel_keyboard())
